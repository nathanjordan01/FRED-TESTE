...
const scoreProducts = (products, searchInput) => {
  ...
  if (typeof entry === 'string') {
    ...
    extraTerms.forEach(et => {
      if (insight.includes(et)) {
        terms.push({ val: et, weight: 40 });
      }
    });
    // Bitola detection in insight (Generoso)
    Object.entries(BITOLA_MAP).forEach(([dash, inches]) => {
  ...
  return products.map(p => {
    ...
    const hasPenaltyKeyword = penalidades.filter(p => !bonusKeywords.includes(p)).some(p => descricao.includes(p));
    
    if (hasPenaltyKeyword && !hasCategoryKeyword) return { ...p, score: 0 };
